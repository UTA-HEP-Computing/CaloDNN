...
# Parse the Arguments
execfile("CaloDNN/RegressionArguments.py")
...
if BuildModel and not MyModel.Model:
    ...
    if HCAL and ECAL:
        MyModel = MergerRegEModel(Name + "_Merged", [ECALModel, HCALModel],
                                  WeightInitialization,
                                  OutputBase=OutputBase)
...
if Analyze:
    ...
    result, NewMetaData = RegressionAnalysis(MyModel,
                                             [Test_X_ECAL, Test_X_HCAL], Test_Y,
                                             BatchSize)
